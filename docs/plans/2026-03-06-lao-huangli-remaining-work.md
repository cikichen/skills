...
- 节气窗口已经升级为 `Skyfield + JPL ephemeris` 的天文时刻输出，并已增加节气表/节气导航结构。
- 农历核心已经接入“天文新月 -> 月份表”的最小骨架，可输出 `monthStartDate`、`monthDayCount`、`zhongQi` 等元数据。
- 当前仍未按标准完整落地“含冬至者为十一月”和“无中气置闰”的全量校验，也还保留了表驱动回退路径。

### 2. 《协纪辨方书》规则层仍不完整

- `good-stars.json`、`bad-stars.json`、`duty-gods.json` 已接入最小可用逻辑，当前主要仍缺更完整的神煞族，而不是空骨架。
- `yi-ji-rules.json` 已覆盖卷十中一批可直接程序化的 `建除` 条目，但仍未完整覆盖各类神煞组合裁决。
- `market-folk-v1` 已完成一批高频派生，但还不是完整市售挂历全量规则。
